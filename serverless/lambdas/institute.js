...
const signup = async (event) => {
  ...
  if (!institute) {
    ...
    const newInStitutes = await InstituteModel.create({
      uid: userUid,
      email,
      name,
      password,
    ...
    return {
      statusCode: 200,
      body: JSON.stringify(
        ApiResponse.error(HttpStatus.CONFLICT, 'User with email already exists')
      ),
    };
  }
};
...
const updateOTP = async (event) => {
  ...
  try {
    await InstituteModel.findOneAndUpdate(
      { email: email },
      { $set: { OTPResetPassword: hashedOTP } },
      { upsert: false }
    );
    ...
    return {
      statusCode: 200,
      body: JSON.stringify(
        ApiResponse.success(HttpStatus.OK, 'Email sent successfully!', token)
      ),
    };
  } catch (error) {
    return {
  ...
  try {
    ...
    if (!passwordMatch) {
      return {
        statusCode: 200,
        body: JSON.stringify(
          ApiResponse.error(HttpStatus.UNAUTHORIZED, 'Invalid old password')
        ),
      };
    }
    ...
    return {
      statusCode: 200,
      body: JSON.stringify(
        ApiResponse.success(HttpStatus.OK, 'Password changed successfully')
      ),
    };
  } catch (error) {
    console.error(error);
    return {
      statusCode: 200,
      body: JSON.stringify(
        ApiResponse.error(HttpStatus.INTERNAL_SERVER_ERROR, 'An error occurred')
      ),
    };
  }
};
  ...
  try {
    ...
    if (email) {
      ...
      if (currentTime <= tokenExpirationTime) {
        ...
        const resultOTP = await bcryptjs.compare(
          OTP.toString(),
          institute.OTPResetPassword
        );
        ...
        if (resultOTP) {
          ...
          return {
            statusCode: 200,
            body: JSON.stringify(
              ApiResponse.error(HttpStatus.UNAUTHORIZED, 'OTP code is invalid')
            ),
          };
        }
      } else {
      ...
      return {
        ...
        body: JSON.stringify(
          ApiResponse.error(
            HttpStatus.UNAUTHORIZED,
            'Email information not found in token'
          )
        ),
      };
    }
  ...
  try {
    ...
    if (existUser) {
      ...
      return {
        statusCode: 200,
        body: JSON.stringify(
          ApiResponse.success(HttpStatus.OK, 'Account Deleted Successfully!')
        ),
      };
    } else {
      return {
      ...
      if (currentTime <= tokenExpirationTime) {
        ...
        if (resultOTP) {
          ...
          return {
            statusCode: 200,
            body: JSON.stringify(
              ApiResponse.error(HttpStatus.UNAUTHORIZED, 'Invalid OTP code')
            ),
          };
        }
      } else {
