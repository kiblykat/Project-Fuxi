    ...
    if (response) {
      return {
        ...
        body: JSON.stringify(
          ApiResponse.success(
            HttpStatus.OK,
            'Get all playlist in profile success',
            response
          )
        ),
      };
      ...
      return {
        statusCode: 200,
        body: JSON.stringify(
          ApiResponse.error(HttpStatus.NOT_FOUND, 'Playlist not found')
        ),
      };
    }
  } catch (error) {
...
const getAllPlayListByProfileId = async (event) => {
  ...
  try {
    ...
    const response = await PlaylistModel.find({
...
const removeTrackInPlaylist = async (event) => {
  ...
  try {
    ...
    const filteredTracks = existingPlaylist.tracks.filter(
      (existingTrackId) => existingTrackId.toString() !== trackId
    );
  ...
  try {
    ...
    if (deletePlaylist) {
      return {
        statusCode: 200,
        body: JSON.stringify(
          ApiResponse.success(HttpStatus.NO_CONTENT, 'Delte playlist success')
        ),
      };
    } else {
      return {
...
const deleteAllPlaylist = async (event) => {
  ...
  try {
    ...
    const deletedPlaylists = await PlaylistModel.deleteMany({
      profileId: profileId,
    });
    ...
    if (deletedPlaylists.deletedCount > 0) {
      ...
      return {
        statusCode: 200,
        body: JSON.stringify(
          ApiResponse.success(HttpStatus.NO_CONTENT, 'Delete playlist success')
        ),
      };
    } else {
...
const getSuggestionsInPlaymedia = async (event) => {
  ...
  const response = await ProfileReactModal.findOne({
    profileId: new mongoose.Types.ObjectId(profileId),
  });
  ...
  const filterTrackDislike = response.reactTracks.filter(
    (item) =>
      item.preference === 'dislike' || item.preference === 'strongly dislike'
  );
  ...
  try {
    ...
    const filteredUniqueTracksArray = uniqueTracksArray.filter(
      (track) =>
        !filteredTrackIds.some((id) => id.toString() === track.toString())
    );
    ...
    if (preference === 'like' || preference === 'strongly like') {
      ...
      const filterTrackDislike = existingReactProfile.reactTracks.filter(
        (item) =>
          ...
          item.preference === 'strongly dislike'
      );
...
const randomNextTrack = async (event) => {
  ...
  try {
    ...
    return {
      statusCode: 200,
      body: JSON.stringify(
        ApiResponse.success(HttpStatus.OK, 'Random next track', randomTrack[0])
      ),
    };
  } catch (error) {
    // Log any errors and return a server error response
...
const createOrUpdateInitialPlaylistWhenChangeProfile = async (
  profileId,
  genres
) => {
  ...
  for (const genre of genres) {
    ...
    while (
      ...
      tracksFromGenre < maxTracksPerGenre
    ) {
