...
const getReactTrackByProfileId = async (event) => {
  ...
  try {
    const response = await ProfileReactModal.find({
...
const getLikeTrackByProfileId = async (event) => {
  ...
  try {
    ...
    const tracksLike = response[0].reactTracks.filter(
      (item) =>
        item.preference === 'like' || item.preference === 'strongly like'
    );

    // Remove duplicate tracks based on their IDs
    const uniqueTracks = Array.from(
      new Set(tracksLike.map((track) => track._id))
    ).map((_id) => {
...
const createProfileReact = async (profileId, reactTracks) => {
  ...
  try {
    const response = await ProfileReactModal.create({
      profileId: profileId,
...
const getReactTrackByTrackId = async (event) => {
  ...
  try {
    ...
    const desiredObject = response.reactTracks.find((item) =>
      item.track.equals(new ObjectId(trackId))
    );
    if (response) {
      return {
        statusCode: 200,
      ...
      return {
        statusCode: 200,
        body: JSON.stringify(
          ApiResponse.error(HttpStatus.NOT_FOUND, 'React track not found')
        ),
      };
    }
  } catch (error) {
  ...
  try {
    ...
    if (updatedReactTrack) {
      return {
        statusCode: 200,
        body: JSON.stringify(
          ApiResponse.success(HttpStatus.OK, 'Added a new react track success')
        ),
      };
    } else {
      return {
    ...
    if (updatedReactTrack) {
      return {
        ...
        body: JSON.stringify(
          ApiResponse.success(
            HttpStatus.OK,
            'Updated react track preference success'
          )
        ),
      };
    } else {
  ...
  try {
    ...
    if (updatedProfile) {
      return {
        statusCode: 200,
        body: JSON.stringify(
          ApiResponse.success(HttpStatus.OK, 'Removed a react track success')
        ),
      };
    } else {
      return {
...
const deleteProfileReact = async (event) => {
  ...
  try {
    const existingReactTrack = await ProfileReactModal.deleteOne({
      profileId: new ObjectId(profileId),
    });
    ...
    if (existingReactTrack.deletedCount > 0) {
      return {
        statusCode: 200,
        body: JSON.stringify(
          ApiResponse.success(HttpStatus.OK, 'Deleted react profile success')
        ),
      };
    } else {
      return {
        statusCode: 200,
        body: JSON.stringify(
          ApiResponse.error(HttpStatus.NOT_FOUND, 'React profile not found')
        ),
      };
    }
  } catch (error) {
