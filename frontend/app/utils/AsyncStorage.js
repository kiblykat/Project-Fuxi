...
export const addItemToAsyncStorageArray = async (key, item) => {
  try {
    ...
    console.log(
      'The element has been added to the array and saved to AsyncStorage'
    );
  } catch (error) {
    console.error(
      'Error when adding elements to array and saving to AsyncStorage:',
      error
    );
  }
};
...
export const removeItemFromAsyncStorageArray = async (key, item) => {
  try {
    ...
    console.log(
      'The element has been removed from the array and updated into AsyncStorage'
    );
  } catch (error) {
    console.error(
      'Error removing element from array and updating to AsyncStorage:',
      error
    );
  }
};
...
export const isItemInAsyncStorageArray = async (key, item) => {
  try {
    ...
    if (isItemInArray) {
      ...
      console.log(
        `Element ${item} is not in the array in AsyncStorage (${key})`
      );
    }
