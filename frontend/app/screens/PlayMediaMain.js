...
const PlayMediaMain = ({
  ...
  const handlePrevTrack = async () => {
    if (Object.keys(dataTracks).length !== 0) {
      await handleTrackPress(
        await findPreviousTrack(playlistId, dataTracks, selectSound._id)
      );
    }
  };
  ...
  const handleNextTrack = async () => {
    if (Object.keys(dataTracks).length !== 0) {
      await handleTrackPress(
        await findNextTrack(playlistId, dataTracks, selectSound._id, isCircle)
      );
    }
  };

  const removeTrack = async () => {
    const trackArrRemove = dataTracks.filter(
