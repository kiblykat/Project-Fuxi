...
const DislikedSongsScreen = ({
  ...
  const filteredDislike = dataReactTracks?.filter(
    (item) => item.preference === 'dislike'
  );

  const filteredStronglyDislike = dataReactTracks?.filter(
    (item) => item.preference === 'strongly dislike'
  );
                  ...
                  <CustomGridLayout
                    data={filteredStronglyDislike.map((item, index) => (
                      <ReactSongItem
                        ...
                        dataTracksOrigin={filteredStronglyDislike.map(
                          (item) => item.track
                        )}
                      />
                    ))}
                  ...
                  <CustomGridLayout
                    data={filteredDislike.map((item, index) => (
                      <ReactSongItem
                        ...
                        setIsDialogVisible={setIsDialogVisible}
                        setDialogProps={setDialogProps}
                        dataTracksOrigin={filteredDislike.map(
                          (item) => item.track
                        )}
                      />
                    ))}
