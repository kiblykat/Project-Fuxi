...
export const getPlaylistById = async (playlistId) => {
  try {
    const response = await axios.get(
      `${apiUrl}/dev/playlist?playlistId=${playlistId}`
    );
...
export const getAllPlayListByProfileId = async (profileId) => {
  try {
    const response = await axios.get(
      `${apiUrl}/dev/playlists?profileId=${profileId}`
    );
    return response.data;
  } catch (error) {
    console.error(error);
...
export const getSuggestionsInPlaymedia = async (
  profileId,
  playlistId,
  artist,
  language,
...
export const randomNextTrack = async (profileId, trackIds, trackId) => {
  try {
    const response = await axios.post(
      `${apiUrl}/dev/playlist/random-next-track`,
      {
        profileId: profileId,
        trackIds: trackIds,
        trackId: trackId,
